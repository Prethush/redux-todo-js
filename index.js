...
function handleTodo(event) {
  if (event.keyCode === 13 && event.target.value) {
    store.dispatch({ type: "add", todo: event.target.value, isCompleted: false });
    event.target.value = "";
  }
}
...
function createUI(todos) {
  ...
  todos.forEach((t, i) => {
    ...
    li.classList.add("flex", "justify-between", "my-2", "items-center", "text-xl");
    ...
    radio.addEventListener("click", () => {
      store.dispatch({ type: "iscompleted", isCompleted: !t.isCompleted, id: i });
    })
...
function reducer(state = [], action) {
  switch (action.type) {
    case "add":
      return state.concat({ todo: action.todo, isCompleted: action.isCompleted });
      break;
    case "delete":
